...
struct PairingSheet: View {
  ...
  private var header: some View {
    HStack {
      VStack(alignment: .leading, spacing: 2) {
        Text("Add Wireless Device").font(.title2.bold())
        Text("On your Android device: Settings → Developer options → Wireless debugging").font(.caption).foregroundStyle(.secondary)
      }
      Spacer()
  ...
  private var bindStatus: (String?, Bool) -> Void {
    { msg, isError in statusMessage = msg; statusIsError = isError }
  }
}
...
private struct QRPane: View {
  ...
  private var qrImage: Image {
    ...
    filter.message = Data(qrPayload.utf8); filter.correctionLevel = "M"
    ...
    let nsImage = NSImage(cgImage: cg, size: NSSize(width: cg.width, height: cg.height)); return Image(nsImage: nsImage)
  }
}
...
private struct Step: View {
  ...
  var body: some View {
    HStack(alignment: .top, spacing: 8) {
      Text("\(n)").font(.caption.bold()).frame(width: 20, height: 20).background(Color.accentColor.opacity(0.15), in: Circle())
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      if candidates.isEmpty {
        VStack(spacing: 6) { ProgressView().scaleEffect(0.7); Text("Searching for devices…").font(.callout).foregroundStyle(.secondary) }
        ...
        List(selection: $selection) {
          ForEach(candidates) { ep in
            HStack {
              ...
              VStack(alignment: .leading) { Text(ep.displayName).font(.callout); Text("\(ep.host):\(ep.port)").font(.caption.monospaced()).foregroundStyle(.secondary) }
            }.tag(ep.id)
          }
        }.listStyle(.bordered).frame(minHeight: 120)
      }
      HStack {
        TextField("123456", text: $code).textFieldStyle(.roundedBorder).frame(width: 140).font(.body.monospaced())
...
private struct ManualIPPane: View {
    ...
    VStack(alignment: .leading, spacing: 16) {
      ...
      VStack(alignment: .leading, spacing: 10) {
        HStack(spacing: 4) {
          ...
          TextField("port", text: $port).textFieldStyle(.roundedBorder).frame(width: 90).font(.body.monospaced())
        ...
        HStack(spacing: 4) {
          ...
          TextField("123456 (optional)", text: $pairCode).textFieldStyle(.roundedBorder).frame(width: 180).font(.body.monospaced())
  ...
  private func go() {
    ...
    working = true; status(nil, false)
    let needsPair = !pairCode.isEmpty; let code = pairCode
    Task {
      defer { Task { @MainActor in working = false } }
      do {
        if needsPair {
          ...
          await MainActor.run { SessionCoordinator.shared.trustDevice("\(host):\(portNum)") }

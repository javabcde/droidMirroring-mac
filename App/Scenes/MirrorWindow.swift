...
final class MirrorWindowController: NSWindowController {
  ...
  override func close() {
    ...
    Task { if recorder.isRecording { await withCheckedContinuation { (c: CheckedContinuation<Void, Never>) in recorder.stop { _ in c.resume() } } }; if isScreenOff, let w = await session.control { try? await w.send(.setScreenPowerMode(2)) }; if let s = deviceSerial, let v = savedDeviceVolume { try? await adb.shell("media volume --stream 3 --set \(v)", serial: s); savedDeviceVolume = nil }; await session.stop() }; saveWindowFrame(); super.close()
  }
  ...
  @objc private func wakeDevice() { isScreenOff = false; window?.toolbar?.validateVisibleItems(); Task { if let w = await session.control { try? await w.send(.setScreenPowerMode(2)) } } }
  @objc private func sendBack() { Task { if let w = await session.control { try? await w.send(.backOrScreenOn(action: .down)); try? await w.send(.backOrScreenOn(action: .up)) } } }
  ...
  @objc private func rotateDevice() { Task { if let w = await session.control { try? await w.send(.rotateDevice()) } } }
  ...
  @objc private func toggleScreenOff() { let tf = !isScreenOff; isScreenOff = tf; window?.toolbar?.validateVisibleItems(); Task { if let w = await session.control { try? await w.send(.setScreenPowerMode(tf ? 0 : 2)) } } }
  private func startScreenStatePoller() { screenStatePoller?.invalidate(); screenStatePoller = Timer.scheduledTimer(withTimeInterval: 3, repeats: true) { [weak self] _ in guard let self, let s = self.deviceSerial else { return }; Task.detached { [weak self] in guard let self else { return }; let off = await self.queryDeviceScreenOff(serial: s); let ao = await self.autoScreenOffEnabled; await MainActor.run { if self.isScreenOff != off { self.isScreenOff = off; self.window?.toolbar?.validateVisibleItems() }; if ao, !off { Task { if let w = await self.session.control { try? await w.send(.setScreenPowerMode(0)) } } } } } } }
  ...
  private func sendKey(action: KeyEventAction, code: Int32) { Task { if let w = await session.control { try? await w.send(.keycode(code, action: action)) } } }
  ...
  private func handleMouseMoved(_ event: NSEvent) { guard let w = self.window else { return }; if event.locationInWindow.y > w.frame.height - 80 { setChrome(revealed: true) }; scheduleHideChrome() }
  private func setChrome(revealed: Bool, animated: Bool = true) { chromeHideTimer?.invalidate(); chromeHideTimer = nil; if chromeRevealed == revealed { return }; chromeRevealed = revealed; guard let w else { return }; let a: CGFloat = revealed ? 1.0 : 0.0; let btns: [NSButton?] = [w.standardWindowButton(.closeButton), w.standardWindowButton(.miniaturizeButton), w.standardWindowButton(.zoomButton)]; if animated { NSAnimationContext.runAnimationGroup { c in c.duration = 0.18; btns.forEach { $0?.animator().alphaValue = a }; overlayBar?.animator().alphaValue = a } } else { btns.forEach { $0?.alphaValue = a }; overlayBar?.alphaValue = a } }
  ...
  private func applyDimensions(_ size: CGSize) { guard size.width > 0, size.height > 0 else { return }; let ch = currentDeviceSize != size; currentDeviceSize = size; eventView.deviceDimensions = size; guard let w = self.window else { return }; let aspect = aspectRatio(for: size); w.contentAspectRatio = aspect; if !hasSetInitialFrame { hasSetInitialFrame = true; setInitialFrame(deviceSize: size, window: w); restoreWindowFrame(); return }; guard ch else { return }; let of = w.frame; let oc = w.contentRect(forFrameRect: of).size; let oa = max(1, oc.width * oc.height); let sc = sqrt(oa / (aspect.width * aspect.height)); let nc = CGSize(width: aspect.width * sc, height: aspect.height * sc); let nf = w.frameRect(forContentRect: NSRect(origin: .zero, size: nc)); var ff = nf; ff.origin = CGPoint(x: of.midX - ff.width / 2, y: of.midY - ff.height / 2); w.setFrame(ff, display: true, animate: false); w.invalidateShadow() }
  ...
  private func showAlert(_ error: Error) { let a = NSAlert(error: error); if let w = window { a.beginSheetModal(for: w) } else { a.runModal() } }
}
